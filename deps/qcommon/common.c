...
void *Z_TagMalloc(int size, int tag) {
	...
	do {
		if (rover == start) {
			...
			Com_Error(ERR_FATAL, "Z_Malloc: failed on allocation of %i bytes from the %s zone", size,
...
void Com_Frame(void) {
	...
	if (com_speeds->integer) {
		...
		Com_Printf("frame:%i all:%3i sv:%3i ev:%3i cl:%3i gm:%3i rf:%3i bk:%3i\n", com_frameNumber, all, sv, ev, cl,
				   time_game, time_frontend, time_backend);
	}
...
qboolean Com_Memcmp(const void *src0, const void *src1, const unsigned int count) {
	...
	if (count >= 16) {
		...
		for (i = 0; i < nm2; i += 4) {
			unsigned int tmp =
				(dw[i + 0] - sw[i + 0]) | (dw[i + 1] - sw[i + 1]) | (dw[i + 2] - sw[i + 2]) | (dw[i + 3] - sw[i + 3]);
			if (tmp)
				return qfalse;
		}
	}
