...
int BotWalkInDirection(bot_movestate_t *ms, vec3_t dir, float speed, int type) {
	...
	if (ms->moveflags & MFL_ONGROUND) {
		...
		origin[2] += 0.5;
		AAS_PredictClientMovement(&move, ms->entitynum, origin, presencetype, qtrue, velocity, cmdmove, cmdframes,
								  maxframes, 0.1f, stopevent, 0, qfalse); // qtrue);
...
bot_moveresult_t BotTravel_Grapple(bot_movestate_t *ms, aas_reachability_t *reach) {
	...
	if (ms->moveflags & MFL_ACTIVEGRAPPLE) {
	   ...
	   //
		...
		result.flags |= MOVERESULT_MOVEMENTVIEW;
		//
		if (dist < 5 && fabs(AngleDiff(result.ideal_viewangles[0], ms->viewangles[0])) < 2 &&
...
bot_moveresult_t BotTravel_RocketJump(bot_movestate_t *ms, aas_reachability_t *reach) {
	...
	//
	if (dist < 5 && fabs(AngleDiff(result.ideal_viewangles[0], ms->viewangles[0])) < 5 &&
	   ...
	   //
			...
			case TRAVEL_FUNCBOB:
				*result = BotTravel_FuncBobbing(ms, &reach);
				break;
			case TRAVEL_JUMPER:
				*result = BotTravel_Jumper(ms, &reach);
				break; // cyr
			default: {
