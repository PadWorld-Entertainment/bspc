					...
					if (AAS_PointAreaNum(trace.endpos) == area2num) {
						...
						if (i >= numareas) {
							...
							lreach->traveltime = aassettings.rs_startwalkoffledge +
												 fabs(ground_bestdist) * 50 / aassettings.phys_gravity;
...
int AAS_Reachability_Jump(int area1num, int area2num) {
		...
		for (j = 0; j < area2->numfaces; j++) {
			...
			for (k = 0; k < face1->numedges; k++) {
				...
				for (l = 0; l < face2->numedges; l++) {
					...
					plane2 = &aasworld.planes[face2->planenum];
					//
					bestdist = AAS_ClosestEdgePoints(v1, v2, v3, v4, plane1, plane2, beststart, bestend, beststart2,
													 bestend2, bestdist);
				} // end for
			}	  // end for
		}		  // end for
	...
	if (bestdist > 4 && bestdist < maxjumpdistance) {
		...
		for (i = 0; i < 3; i++) {
			...
			//
			AAS_PredictClientMovement(&move, -1, beststart, PRESENCE_NORMAL, qtrue, velocity, cmdmove, 3, 30, 0.1f,
									  stopevent, 0, qfalse);
...
void CreateReachesForSprayTele(vec3_t origin, vec3_t mins, vec3_t maxs) {
	...
	if (!areas)
		botimport.Print(PRT_MESSAGE, "spraytele not in any area\n");
	//
	for (link = areas; link; link = link->next_area) {
		i = link->areanum;
...
// Changes Globals:		-
//===========================================================================
aas_lreachability_t *AAS_FindFaceReachabilities(vec3_t *facepoints, int numpoints, aas_plane_t *plane,
												int towardsface) {
	int i, j, k, l;
		...
		for (j = 0; j < area->numfaces; j++) {
			...
			for (k = 0; k < face->numedges; k++) {
				...
				for (l = 0; l < numpoints; l++) {
					...
					dist = AAS_ClosestEdgePoints(v1, v2, v3, v4, faceplane, plane, beststart, bestend, beststart2,
												 bestend2, bestdist);
					if (dist < bestdist) {
						bestfacenum = facenum;
...
void AAS_Reachability_FuncBobbing(void) {
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
		...
		Log_Write("funcbob model %d, start = {%1.1f, %1.1f, %1.1f} end = {%1.1f, %1.1f, %1.1f}\n", modelnum,
				  move_start[0], move_start[1], move_start[2], move_end[0], move_end[1], move_end[2]);
		//
#ifndef BSPC
		/*
			...
			for (i = 0; i < area2->numfaces; i++) {
				...
				if (ret && speed < 150) {
					...
					dir[2] = 0;
					{
						...
						//
						AAS_PredictClientMovement(&move, -1, areastart, PRESENCE_NORMAL, qfalse, velocity, cmdmove, 30,
												  30, 0.1f,
...
void AAS_SetWeaponJumpAreaFlags(void) {
	...
	for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
		if (!AAS_ValueForBSPEpairKey(ent, "classname", classname, MAX_EPAIRKEY))
			continue;
		if (!strcmp(classname, "item_armor_body") || !strcmp(classname, "item_armor_combat") ||
			!strcmp(classname, "item_health_mega") || !strcmp(classname, "weapon_grenadelauncher") ||
				...
				for (l = 0; l < face2->numedges; l++) {
					...
					if (abs(edge1num) == abs(edge2num)) {
						...
						if (aassettings.rs_maxfallheight &&
							fabs(mid[2] - trace.endpos[2]) > aassettings.rs_maxfallheight)
							break;
						//
						lreach = AAS_AllocReachability();
						if (!lreach)
						...
						lreach->traveltype = TRAVEL_WALKOFFLEDGE;
						lreach->traveltime = aassettings.rs_startwalkoffledge +
											 fabs(mid[2] - trace.endpos[2]) * 50 / aassettings.phys_gravity;
...
void CheckForJumpPadReaches(aas_lreachability_t *myreach) {
	...
	if (!AAS_GetJumpPadInfo(bestent, areastart, absmins, absmaxs, velocity))
		return;
	// get the areas the jump pad brush is in
	areas = AAS_LinkEntityClientBBox(absmins, absmaxs, -1, PRESENCE_CROUCH);
...
void myQ_strncpyz(char *dest, const char *src, int destsize) {
	...
	if (!dest)
		botimport.Print(PRT_MESSAGE, "Q_strncpyz: NULL dest");
	if (!src)
		botimport.Print(PRT_MESSAGE, "Q_strncpyz: NULL src");
	if (destsize < 1)
		botimport.Print(PRT_MESSAGE, "Q_strncpyz: destsize < 1");
...
void expat_parseAway(char *xmlReachFile) {
	...
	if (!p) {
		botimport.Print(PRT_MESSAGE, "parser: geht net \n");
		return;
	}
