...
void AAS_JumpReachRunStart(aas_reachability_t *reach, vec3_t runstart) {
	...
	//
	AAS_PredictClientMovement(&move, -1, start, PRESENCE_NORMAL, qtrue, vec3_origin, cmdmove, 1, 2, 0.1f,
...
void AAS_JumperReachRunStart(aas_reachability_t *reach, vec3_t runstart) {
	...
	VectorScale(hordir, 400, cmdmove);
	//
	AAS_ClientMovementPrediction(&move, -1, start, PRESENCE_NORMAL, qtrue, vec3_origin, cmdmove, 1, 2, 0.1f,
								 SE_ENTERWATER | SE_ENTERSLIME | SE_ENTERLAVA | SE_HITGROUNDDAMAGE | SE_GAP, 0, mins,
								 maxs, qfalse, 1); // cyr, jumper
	VectorCopy(move.endpos, runstart);
	// don't enter slime or lava and don't fall from too high
	if (move.stopevent & (SE_ENTERSLIME | SE_ENTERLAVA | SE_HITGROUNDDAMAGE)) {
		VectorCopy(start, runstart);
...
int AAS_ClientMovementHitBBox(struct aas_clientmove_s *move, int entnum, vec3_t origin, int presencetype, int onground,
	...
	return AAS_ClientMovementPrediction(move, entnum, origin, presencetype, onground, velocity, cmdmove, cmdframes,
										maxframes, frametime, SE_HITBOUNDINGBOX, 0, mins, maxs, visualize, 0); // cyr
...
void AAS_TestMovementPrediction(int entnum, vec3_t origin, vec3_t dir) {
	...
	AAS_PredictClientMovement(&move, entnum, origin, PRESENCE_NORMAL, qtrue, velocity, cmdmove, 13, 13, 0.1f,
							  SE_HITGROUND, 0, qtrue); // SE_LEAVEGROUND);
