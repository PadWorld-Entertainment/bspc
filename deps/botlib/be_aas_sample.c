...
qboolean AAS_AreaEntityCollision(int areanum, vec3_t start, vec3_t end, int presencetype, int passent,
	...
	for (link = aasworld.arealinkedentities[areanum]; link; link = link->next_ent) {
		...
		if (link->entnum == passent)
			continue;
		//
		if (AAS_EntityCollision(link->entnum, start, boxmins, boxmaxs, end, CONTENTS_SOLID | CONTENTS_PLAYERCLIP,
								&bsptrace)) {
			collision = qtrue;
		} // end if
	}	  // end for
	...
	while (1) {
	   ...
	   // if can't enter the area because it hasn't got the right presence type
			if (!(aasworld.areasettings[-nodenum].presencetype & presencetype)) {
				...
				if (tstack_p->start[0] == start[0] && tstack_p->start[1] == start[1] &&
	   ...
	   // the node to test against
		...
		switch (plane->type) { /*FIXME: wtf doesn't this work? obviously the axial node planes aren't always facing
								  positive!!!
...
int AAS_TraceAreas(vec3_t start, vec3_t end, int *areas, vec3_t *points, int maxareas) {
	...
	while (1) {
	   ...
	   // the node to test against
		...
		switch (
			plane->type) { /*FIXME: wtf doesn't this work? obviously the node planes aren't always facing positive!!!
							  //check for axial planes
							  case PLANE_X:
							  {
								  front = cur_start[0] - plane->dist;
