...
int TH_FindVertex(vec3_t v) {
	...
	for (vertex = thworld.vertexhash[h]; vertex; vertex = vertex->hashnext) {
		if (fabs(vertex->v[0] - vert[0]) < VERTEX_EPSILON && fabs(vertex->v[1] - vert[1]) < VERTEX_EPSILON &&
...
int TH_FindTriangle(int verts[3]) {
	...
	for (tri = thworld.trianglehash[hashvalue]; tri; tri = tri->next) {
		for (i = 0; i < 3; i++) {
			if (abs(tri->edges[i]) != abs(edges[0]) && abs(tri->edges[i]) != abs(edges[1]) &&
				abs(tri->edges[i]) != abs(edges[2]))
				break;
		} // end for
		if (i >= 3)
