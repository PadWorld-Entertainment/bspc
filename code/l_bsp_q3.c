...
void Q3_FreeMaxBSP(void) {
	...
	q3_numFogs = 0;
	if (q3r_dbrushsides)
		FreeMemory(q3r_dbrushsides);
	q3r_dbrushsides = NULL;
	q3r_numbrushsides = 0;
	if (q3r_drawVerts)
		FreeMemory(q3r_drawVerts);
	q3r_drawVerts = NULL;
	q3r_numDrawVerts = 0;
	if (q3r_drawSurfaces)
		FreeMemory(q3r_drawSurfaces);
...
void Q3_LoadBSPFile(struct quakefile_s *qf) {
	...
	q3_numleafsurfaces =
		Q3_CopyLump(header, Q3_LUMP_LEAFSURFACES, (void *)&q3_dleafsurfaces, sizeof(q3_dleafsurfaces[0]));
...
void Q3_WriteBSPFile(char *filename) {
	...
	Q3_AddLump(bspfile, header, Q3_LUMP_LEAFSURFACES, q3_dleafsurfaces,
			   q3_numleafsurfaces * sizeof(q3_dleafsurfaces[0]));
