...
void CheckBSPBrush(bspbrush_t *brush) {
	...
	for (i = 0; i < brush->numsides; i++) {
		for (j = 0; j < brush->numsides; j++) {
			...
			//
			if (WindingsNonConvex(brush->sides[i].winding, brush->sides[j].winding, plane1->normal, plane2->normal,
	...
	for (i = 0; i < brush1->numsides; i++) {
		...
		for (j = 0; j < brush2->numsides; j++) {
			...
			if (side1->planenum == side2->planenum) {
				if (side1->texinfo != TEXINFO_NODE && side2->texinfo != TEXINFO_NODE &&
			...
			//
			if (WindingsNonConvex(side1->winding, side2->winding, plane1->normal, plane2->normal, plane1->dist,
								  plane2->dist)) {
				return NULL;
			} // end if
		}	  // end for
