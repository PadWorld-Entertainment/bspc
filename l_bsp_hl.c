...
void HL_LoadBSPFile(char *filename, int offset, int length) {
	...
	hl_nummarksurfaces =
		HL_CopyLump(HL_LUMP_MARKSURFACES, hl_dmarksurfaces, sizeof(hl_dmarksurfaces[0]), HL_MAX_MAP_MARKSURFACES);
...
unsigned ArrayUsage(char *szItem, int items, int maxitems, int itemsize) {
	...
	qprintf("%-12s  %7i/%-7i  %7i/%-7i  (%4.1f%%)", szItem, items, maxitems, items * itemsize, maxitems * itemsize,
			percentage);
	if (percentage > 80.0)
		qprintf("VERY FULL!\n");
...
void HL_PrintBSPFileSizes(void) {
	...
	totalmemory +=
		ArrayUsage("marksurfaces", hl_nummarksurfaces, ENTRIES(hl_dmarksurfaces), ENTRYSIZE(hl_dmarksurfaces));
